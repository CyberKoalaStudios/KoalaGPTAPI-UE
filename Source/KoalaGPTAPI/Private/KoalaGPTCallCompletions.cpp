...
#include "KoalaGPTCallCompletions.h"
...
UKoalaGPTCallCompletions::UKoalaGPTCallCompletions()
{
}
...
UKoalaGPTCallCompletions* UKoalaGPTCallCompletions::KoalaGPTCallCompletions(FString promptInput, FCompletionSettings settingsInput)
{
	UKoalaGPTCallCompletions* BPNode = NewObject<UKoalaGPTCallCompletions>();
	BPNode->prompt = promptInput;
	BPNode->settings = settingsInput;
	return BPNode;
}

void UKoalaGPTCallCompletions::Activate()
{
	FString _apiKey;
	...
	// set headers
	FString url = FString::Printf(TEXT("https://gpt.cyberkoala.ru/backend-api/v2/unreal"), *apiMethod);
	...
	{
		HttpRequest->OnProcessRequestComplete().BindUObject(this, &UKoalaGPTCallCompletions::OnResponse);
	}
	else
	{
...
void UKoalaGPTCallCompletions::OnResponse(FHttpRequestPtr Request, FHttpResponsePtr Response, bool WasSuccessful)
{
	if (!WasSuccessful)
	{
	...
	if (FJsonSerializer::Deserialize(reader, responseObject))
	{
		...
		KoalaGPTParser parser(settings);
